...
let rec private firstOfferingOnOrAfter (unitCode: UnitCode) (semester: Semester) : Semester =
    let unitInfo = StudyPlannerModel.lookup(unitCode:UnitCode) // Obtain the info about the current Unit.
    let sequenceOfUnits = StudyPlannerModel.SemesterSequence semester {year=semester.year+1; offering=Semester2} // Generate a sequence of units from the current Semester to Semester 2 one year ahead.
    let finalFirstSemester = Seq.map (fun x -> x) sequenceOfUnits |> Seq.filter (fun x -> Seq.contains x.offering unitInfo.offered) // Map the sequence of units generated above and filter it to only get the entries that contain Semesters where the unit is offered.
    Seq.head finalFirstSemester // Return the head/first element of the filtered sequence of Semesters.
